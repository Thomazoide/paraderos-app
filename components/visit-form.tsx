...
export default function VisitFormComponent(props: {
  busStop: BusStop;
  workOrder: WorkOrder;
  status: "start" | "finish";
  formID?: number;
  ...
  const updateBusStop = async () => {
    try {
      const busStopEndpoint = `${BACKEND_URL}${ENDPOINTS.busStops}`;
      const busRequestConfig = GetRequestConfig(
        "POST",
        "JSON",
        JSON.stringify({
          ...props.busStop,
      ...
      const busStopUpdateResponse = (await (
        await fetch(busStopEndpoint, busRequestConfig)
      ).json()) as ResponsePayload<BusStop>;
      if (busStopUpdateResponse.error)
        throw new Error(busStopUpdateResponse.message || "Error desconocido");
    } catch (e) {
      Alert.alert(
        e instanceof Error
          ? e.message
          : "Error actualizando datos del paradero",
        "Ocurrió un error al actualizar la ultima visita al paradero actual, por favor intentelo nuevamente en unos momentos...",
      );
    }
  };
  ...
  const SaveFirstPartOfForm = async () => {
    try {
      ...
      if (!textInputData && !cameraRef.current) {
        Alert.alert(
          "Sin datos!",
          "Debe rellenar con una descripción y tomar una foto obligatoriamente...",
        );
        setLoading(false);
        return;
      }
      ...
      if (!routeStringifiedData) {
        Alert.alert(
          "Sin datos de ruta",
          "Probablemente deba tomar una orden nueva...",
          [
            {
              text: "Ir a órdenes",
      ...
      ).json()) as ResponsePayload<VisitForm>;
      if (response.error) throw new Error(response.message);
      await updateBusStop();
      Alert.alert(
        "Formulario creado",
        `Formulario #${response.data!.id} creado con exito.\nPara finalizarlo debe buscarlo en la pestaña de formularios`,
        [
          {
            text: "Volver",
  ...
  const saveLastPartOfForm = async () => {
    try {
      ...
      if (!textInputData && !picData) {
        Alert.alert(
          "sin datos!",
          "Debe llenar el campo de descripcion y tomar una foto obligatoriamente",
        );
        setLoading(false);
        return;
      }
      ...
      const config = GetRequestConfig("POST", "FORM", form, accessToken!);
      const response: ResponsePayload<VisitForm> = await (
        await fetch(endpoint, config)
      ).json();
      if (response.error) throw new Error(response.message);
      if (response.data) {
        ...
        const updateWOConfig = GetRequestConfig(
          "POST",
          "JSON",
        ...
        const updateResponse: ResponsePayload<WorkOrder> = await (
          await fetch(updateWOEndpoint, updateWOConfig)
        ).json();
        if (updateResponse.error) throw new Error(updateResponse.message);
        await updateBusStop();
        if (updateResponse.data?.completada) {
          await AsyncStorage.multiRemove([WORK_ORDER_DATA, ROUTE_DATA]);
          Alert.alert(
            "Órden de trabajo completada",
            "Al completar este formulario completaste la orden de trabajo, ya puedes tomar una orden nueva si es que tienes más asignadas.",
            [
              {
                text: "Cerrar",
                onPress: props.cancelAction,
          ...
          Alert.alert(
            "Formulario cerrado",
            "El formulario ha sido cerrado exitosamente",
            [
              {
                text: "Continuar",
                onPress: props.cancelAction,
      ...
      >
        ...
        <ThemedView style={styles.footerContainer}>
          ...
          >
            ...
            />
            <ThemedText
              style={{ color: Colors[theme].text, fontWeight: "600" }}
            >
              Cancelar
            </ThemedText>
          </TouchableOpacity>
