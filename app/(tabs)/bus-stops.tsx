...
export default function BusStopsScreen() {
  ...
  const handleBusStopSelection = async (busStop: BusStop) => {
    if (!workOrderData || !routeData) {
      Alert.alert(
        "Sin orden/ruta activa",
        "Debe tomar una orden para tener una ruta activa y completar formularios",
        [
          {
            text: "Ir a ordenes",
    ...
    try {
      ...
      const incompletedAssigned = busStop.visitForms!.filter(
        (form) => form.userId === parsedUserData.id && !form.completed,
      );
      if (incompletedAssigned.length > 0)
        throw new Error("MCF;Tiene formularios de este paradero sin completar");
      ...
      if (errorCode === "NUD") {
        ...
        Alert.alert(
          errorMessage,
          "Debe completar los formularios existentes antes de crear otros",
          [
            {
        ...
        Alert.alert(
          "Error desconocido",
          "Vuelva a cargar la aplicación o intentelo más tarde",
        );
      }
    }
  };
  ...
  const onRefresh = async () => {
    try {
      ...
      const response = await fetch(
        `${BACKEND_URL}${ENDPOINTS.busStops}`,
        config,
      );
        ...
        <ThemedView style={styles.listContainer}>
          <View style={styles.searchContainer}>
            <TextInput
              style={[
                styles.searchInput,
                { color: Colors[theme].text, borderColor: Colors[theme].icon },
              ]}
            ...
            {routeData ? (
              <View style={styles.checkboxContainer}>
                <CheckBox
                  ...
                  value={showStopsFromRoute}
                  onValueChange={setShowStopsFromRoute}
                  color={showStopsFromRoute ? Colors[theme].tint : undefined}
                />
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <View
                style={[styles.item, { borderBottomColor: Colors[theme].icon }]}
              >
                <ThemedText type="subtitle">
                  {" "}
                  {routeData &&
                  item.id ===
                    routeData.route_points.filter((id) => id === item.id)[0] ? (
                    <StarIcon color={Colors[theme].text} />
                  ) : null}{" "}
                  {item.codigo}
                </ThemedText>
                <ThemedText>{item.description}</ThemedText>
                <View style={styles.itemFooter}>
                  {!new Set(workOrderData?.stops_visited).has(item.id) ? (
                    <TouchableOpacity
                      style={[
                        styles.formButton,
                        { backgroundColor: Colors[theme].tint },
                      ]}
                      onPress={() => handleBusStopSelection(item)}
                    >
                      <ThemedText
                        style={[
                          {
